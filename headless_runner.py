...
class Config:
    def __init__(self):
        ...
        self.email_to = [
            e.strip() for e in os.environ.get("EMAIL_TO", "").split(",") if e.strip()
        ]
        ...
        self.extra_tickers = [
            t.strip()
            for t in os.environ.get("EXTRA_TICKERS", "").split(",")
            if t.strip()
        ]
...
def infer_queries(command: str) -> List[str]:
            ...
            if raw_kws:
                for kw in raw_kws:
                    ...
                    if not mapped and len(kw) >= 2:
                        _add(
                            f"{kw} 投資"
                            if "投資" not in kw and "分析" not in kw
                            else kw
                        )
                if queries:
                    return queries[:6]
    ...
    return (
        queries[:6] if queries else ["台股 分析", "美股 投資", "ETF 分析", "財經 今日"]
    )
        ...
        for vid in vid_ids:
            ...
            result.append(
                VideoItem(
                    ...
                    published_local=to_taipei_display(
                        safe_str(snippet.get("publishedAt"))
                    ),
    ...
    def _price_cards() -> str:
        ...
        for r in price_results:
            ...
            p_str = (
                f"{price:,.2f}"
                if price >= 1000
                else (f"{price:.2f}" if price >= 10 else f"{price:.4f}")
            )
...
async def run_notebooklm_pipeline(
    ...
    if not _HAS_NOTEBOOKLM:
        ...
        return {
            "summary_md": "（未執行 NotebookLM）",
            "report_md": "（未執行 NotebookLM）",
        }

    source_rows = [
        asdict(v) | {"source_kind": "YouTube", "status_text": "待加入"} for v in videos
    ]
    ...
    return {"summary_md": summary_md, "report_md": report_md}
...
async def main_async(cfg: Config):
    ...
    output_dir = Path("outputs") / datetime.now(timezone(timedelta(hours=8))).strftime(
        "%Y%m%d_%H%M%S"
    )
    ...
    drive_link = upload_to_gdrive(cfg.gdrive_sa_b64, cfg.gdrive_folder_id, html_path)
